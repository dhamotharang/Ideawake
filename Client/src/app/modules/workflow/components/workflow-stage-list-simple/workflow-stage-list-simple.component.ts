import { get, keyBy, forEach, trimEnd, sumBy, round, uniq } from 'lodash';
...
@Component({
  selector: 'app-workflow-stage-list-simple',
  templateUrl: './workflow-stage-list-simple.component.html',
  styleUrls: ['./workflow-stage-list-simple.component.scss']
})
export class WorkflowStageListSimpleComponent implements OnInit, OnChanges {
  @Input() workflow;
  communityId;
  stages = [];
  stagesDetail = [];
  ...
  getStages() {
    this.workflowApiService
      .getAllStages({ workflow: this.workflow.id, isDeleted: false })
      .subscribe((res: any) => {
  ...
  criteriaDescriptionHtml(stage) {
    ...
    forEach(get(detail, 'criteria', []), (value) => {
      html += `<ul class="list-inline mb-0 popoverUserMinWidth row">
            <li class="list-inline-item mr-0 col-7">
              <p class="customFieldText mb-0">${
                value.evaluationCriteria.title
              }</p>
            </li>
            <li class="list-inline-item mr-0 col-5">
              <p class="customFieldText text-center mb-0">${
  ...
  fieldDescriptionHtml(stage) {
    ...
    forEach(get(detail, 'customFields', []), (value, key) => {
      html += `<ul class="list-inline mb-0 popoverUserMinWidth row">
                <li class="list-inline-item mr-0 col-7">
                  <p class="text-medium mb-0">${value.field.title}</p>
                </li>
                <li class="list-inline-item mr-0 col-5">
                  <p class="text-medium text-center mb-0">${value.field.customFieldType.title}</p>
                </li>
              </ul>`;
