...
export class MentionListComponent implements OnInit {
  ...
  private checkBounds() {
    ...
    if (this.windowWidth && left + bounds.width > parseInt(this.windowWidth)) {
      // left = parseInt(this.windowWidth) - (bounds.width - 10);
      left = left - bounds.width;
    }

    if (
      this.windowHeight &&
      top + bounds.height > parseInt(this.windowHeight)
    ) {
      top = top - (bounds.height + 30);
    }
